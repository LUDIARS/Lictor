...
| POST   | `/v1/pr/submit`              | `{repo_path, branch?}`                 | Submit a local PR for the supplied repository without requiring an implementation-session repository binding; `branch`, when provided, must be non-empty |
...
| POST   | `/v1/internal/permission-check` | `{tool_name, tool_input?, permission_mode?, guard_result?}` | PreToolUse hook bridge. Records the attempted call and returns `{deferred:true}` in **every** permission mode — it never emits a decision, so Claude Code stays authoritative and no card is posted here. PreToolUse runs before Claude evaluates its rules, so deciding here would gate calls settings.json already allows. |
| POST   | `/v1/internal/notification` | `{message, cwd?, claude_session_id?}` | Notification hook bridge — the only reliable signal that Claude actually stopped for a human. Posts a permission card to Concordia only for permission-waiting messages (idle nudges are dropped), correlating with the most recent PreToolUse observation. Returns `{ok, kind, matched, posted, request_id}`. |
| POST   | `/v1/internal/permission-response` | `{request_id, decision, reason?}` | Answer a permission request by injecting a fixed key sequence into the open TUI dialog (`{ok:true, via:"keystroke"}`). Cards expire 10 minutes after posting so a late answer cannot key an unrelated dialog. |
