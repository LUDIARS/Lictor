...
| POST   | `/v1/internal/permission-check` | `{tool_name, tool_input?, permission_mode?, guard_result?}` | PreToolUse hook bridge. In Claude's `auto` mode it records the call and returns `{deferred:true}` without a decision — auto stays authoritative and no card is posted. Other modes classify as before (self-processable defer / human confirmation, max 10 min). |
...
| POST   | `/v1/internal/permission-response` | `{request_id, decision, reason?}` | Answer a permission request. Hook-held requests resolve over HTTP; Notification-originated ones are answered by injecting a fixed key sequence into the open TUI dialog (`{ok:true, via:"keystroke"}`). |
