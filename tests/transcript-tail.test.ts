...
// 現行 Claude Code は hook が報告する session_id (= --session-id で渡した uuid) と、
// 実際に書く transcript JSONL のファイル名 uuid が一致しないことがある。 その場合
// pinned path (`<session-id>.jsonl`) は永遠に現れず、 pin 固定 discover も sid ベース
// 再 pin も同じ誤ファイルを指すため中継が一切始まらない。 猶予を過ぎたら mtime discover
// にフォールバックして実 transcript を claim し中継を成立させる (本 PR の主目的)。
test("startTranscriptTail: pinned path が現れないとき猶予後に mtime discover へフォールバックする", async () => {
  const dir = mkdtempSync(join(tmpdir(), "lictor-fallback-"));
  try {
    const sessionId = "lictor-fallback-session";
    ...
    // claude が実際に書く transcript (別 uuid)。
    ...
    // 本番同様、 SessionStart hook は誤った sid (= pinnedUuid) を state へ書く。
    // maybeRepin はそこから `<pinnedUuid>.jsonl` (= pinnedPath) を計算するだけなので
    // 実ファイルへ追従できない → フォールバックが必要、 かつ再 pin はフォールバックを壊さない。
    const statePath = claudeSessionStatePath(dir, sessionId);
    writeFileSync(statePath, pinnedUuid);
    const provider = {
      ...PROVIDERS.claude,
      transcriptDir: () => dir,
      pinnedTranscriptFile: (_cwd: string, uuid: string) => join(dir, `${uuid}.jsonl`),
    ...
    const tail = startTranscriptTail({
      ...
      pinnedTranscriptPath: pinnedPath,
      lictorSessionStatePath: statePath,
      pinFallbackGraceMs: 0, // 猶予 0 = 即フォールバック (テスト短縮)
    });
    try {
      await sleep(700);
      assert.equal(tail.getTranscriptPath(), realPath, "pinned 不在 → 実ファイルへフォールバックして tail");
      ...
      assert.equal(existsSync(`${pinnedPath}.lictor-claim`), false, "存在しない pinned は claim しない");
    } finally {
      tail.stop();
    }
  } finally {
...
// フォールバックの逆: pinned path が猶予内に現れたら、 より新しい decoy があっても
// mtime discover に降りず pinned だけを掴む (= 既存の crosstalk 防護を壊さない)。
test("startTranscriptTail: pinned path が猶予内に現れればフォールバックしない", async () => {
  const dir = mkdtempSync(join(tmpdir(), "lictor-nofallback-"));
  try {
    ...
    const tail = startTranscriptTail({
      cwd: dir,
      sessionId: "lictor-nofallback-session",
      concordiaBaseUrl: "http://127.0.0.1:1",
      provider,
      pinnedTranscriptPath: pinnedPath,
      pinFallbackGraceMs: 0, // 即フォールバック許可でも pinned が在れば pinned 優先
    });
    try {
      await sleep(700);
      assert.equal(tail.getTranscriptPath(), pinnedPath, "pinned が在れば pinned を tail");
