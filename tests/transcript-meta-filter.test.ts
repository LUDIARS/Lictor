...
test("readTranscriptFirstLine: cap 内に改行が無い行は null (途切れ JSON を返さない)", () => {
  // 旧仕様は maxBytes で打ち切った文字列を返していたが、途切れた JSON は
  // originator 施錠 (strict) フィルタの誤判定源になるため「読めない」扱いに変更。
  const dir = mkdtempSync(join(tmpdir(), "lictor-meta-"));
  try {
    const p = join(dir, "one-line.jsonl");
    writeFileSync(p, "a".repeat(20000));
    assert.equal(readTranscriptFirstLine(p, 1024), null);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("巨大な session_meta 先頭行 (>8KiB) でも読めて originator 施錠が通る", () => {
  // codex 0.144.x の session_meta は base_instructions を含み数十 KB になる。
  // 旧上限 8KiB では行が途切れて parse 失敗 → strict モードが自分の rollout を
  // 拒否して中継が全停止した (2026-07-13 実害) の再発防止。
  const dir = mkdtempSync(join(tmpdir(), "lictor-meta-"));
  try {
    const big = metaLine({
      cwd: CWD,
    ...
    const first = readTranscriptFirstLine(p);
    assert.ok(first !== null && first.length > 8192);
    assert.equal(
      codexTranscriptMetaAccepts(first!, { cwd: CWD, expectedOriginator: "lictor:lictor-abc" }),
      true,
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test("cap 超過で改行が見つからない先頭行は null (途切れ JSON を返さない)", () => {
  const dir = mkdtempSync(join(tmpdir(), "lictor-meta-"));
  try {
    const p = join(dir, "rollout-huge.jsonl");
    writeFileSync(p, "{" + "\"a\":\"" + "y".repeat(300_000) + "\"}");
    assert.equal(readTranscriptFirstLine(p), null);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
